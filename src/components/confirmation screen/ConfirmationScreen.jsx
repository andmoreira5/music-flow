...
export default function ConfirmationScreen() {
  ...
  async function confirmAction() {
    if (!selectedItem?.id || !tableSelected || !data[tableSelected]) {
      toast.error("Não foi possível excluir o item.");
      return;
    }

    const updatedItems = data[tableSelected].filter(
      (item) => item.id !== selectedItem.id
    );

    setData((prevData) => ({
      ...prevData,
      [tableSelected]: updatedItems,
    }));
  ...
  return (
    <div
      className={`fixed top-0 left-0 w-screen h-screen flex items-center
         justify-center z-50 transition-opacity duration-500 backdrop-blur-md
    ${
      visibleConfirmationScreen
        ? "opacity-100"
